...
$struct=array(
  ...
  "titulo"=>array("label"=>"Titulo","type"=>"input"),
...
if(isset($misiones[@$_GET['id']])){
  $rec=$misiones[$_GET['id']];
}
...
if(count($_POST)){
  ...
  if(!isset($rec)){
    if(empty($_GET['id'])){
      $_GET['id']=count($misiones);
    }
    $misiones[$_GET['id']]=array();
    //$rec=&$misiones[$_GET['id']];
  }
var_dump($_POST,$struct);
  foreach($_POST as $k=>$v)if(!empty($v) || ($struct[$k]['type']!='file')){
    ...
    $misiones[$_GET['id']][$k]=$v;
  }
  //uDB::save("misiones", $misiones);
  header("Location: misiones_list.php");
  exit;
}
...
?>
        <div class="pure-controls">
            ...
            <button type="button" class="pure-button" onclick="window.location.href='misiones_list.php'">Volver</button>
        </div>
</fieldset>
</form>