...
class tar_file extends archive
{
	...
	function extract_files()
	{
		...
		if ($fp = $this->open_archive())
		{
			...
			while ($block = fread($fp, 512))
			{
				...
				{
					$this->error[] = "This script does not support extracting this type of tar file.".var_export($file['magic'],true);
					break;
				}
				...
				{
					if (!is_dir($file['name']))
						$this->createDir($file['name'], "./", $file['stat'][2]);
				}
				...
				else if ($file['type'] == 2)
				{
					...
					chmod($file['name'], 0744 /* $file['stat'][2]*/ );
				}
				else if ($this->createDir(dirname($file['name'])) && $new = fopen($file['name'], "wb"))
				{
					fwrite($new, fread($fp, $file['stat'][7]));
					$z=(512 - $file['stat'][7] % 512) == 512 ? 0 : (512 - $file['stat'][7] % 512);
          if($z>0)fread($fp, $z); 
					fclose($new);
					chmod($file['name'], 0744  /*$file['stat'][2]*/ );
				}
  ...
  function createDir($path,$base="./",$stat=0755)
  {
    $path=str_replace("\\","/",$path);
    $p=explode("/",$path);
    $dir = $base.$p[0];
    if(!file_exists($dir)) { mkdir($dir, $stat); chmod($dir,$stat); }
    unset($p[0]);
