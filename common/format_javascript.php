...
$data = str_replace("<?php", "<script>", highlight_string("<?php " . $data . "\n?>", true)); 

$data = str_replace(array("<br />"), array("\n"),$data);
$data = str_replace(array('<span style="color: #007700">,</span>'), array(",\n"),$data);
$data = str_replace(array('<span style="color: #007700">({</span>'), array("({\n"),$data);
$data = preg_replace('/\<span style="color: #007700"\>([^<]*?}[^<]*?)<\/span>/',"\n".'$1',$data);
$data = explode("\n", $data);
...
$output = str_replace(array('&lt;?php', '?&gt;', '&nbsp;'), array('', '', ' '), $output); // Add nice and friendly <script> tags around highlighted text
